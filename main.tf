...
resource "azurerm_linux_function_app" "this" {
  name                = "${var.function_app_name_prefix}-${substr(data.archive_file.this.output_sha, 0, 8)}" # trigger redeploy on zip change
  resource_group_name = var.resource_group_name
  location            = var.location
...
resource "azurerm_application_insights" "this" {
  name                = "${var.function_app_name_prefix}-${substr(data.archive_file.this.output_sha, 0, 8)}"
  resource_group_name = var.resource_group_name
  location            = var.location
