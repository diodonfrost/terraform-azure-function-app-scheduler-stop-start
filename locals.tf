locals {
  ...
  # Generate a service plan name if not provided
  # Format: "asp-" + function app name + "-" + random 8-character hex suffix
  # This ensures a meaningful and unique name for the service plan
  generated_service_plan_name = var.service_plan_name == null ? "asp-${var.function_app_name}-${random_id.service_plan_suffix[0].hex}" : var.service_plan_name
}
