run "create_test_infrastructure" {
  ...
  assert {
    condition     = module.stop_container_instances.service_plan_name == "spn-to-stop-${random_pet.suffix.id}"
  ...
  assert {
    condition     = module.start_container_instances.service_plan_name == "spn-to-start-${random_pet.suffix.id}"
    error_message = "Invalid service plan name"
  }
