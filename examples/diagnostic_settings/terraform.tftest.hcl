run "create_test_infrastructure" {
  ...
  assert {
    condition     = module.to_event_hub.service_plan_name == "spn-to-event-hub-${random_pet.suffix.id}"
  ...
  assert {
    condition     = module.to_log_analytic.service_plan_name == "spn-to-log-analytic-${random_pet.suffix.id}"
    error_message = "Invalid service plan name"
  }
  ...
  assert {
    condition     = module.to_storage_account.service_plan_name == "spn-to-storage-account-${random_pet.suffix.id}"
    error_message = "Invalid service plan name"
  }
