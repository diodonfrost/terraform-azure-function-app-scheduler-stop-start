run "create_test_infrastructure" {
  ...
  assert {
    condition     = module.stop_virtual_machines.service_plan_name == "spn-to-stop-${random_pet.suffix.id}"
  ...
  assert {
    condition     = module.start_virtual_machines.resource_group_name == azurerm_resource_group.test.name
    error_message = "Invalid resource group name"
  }
