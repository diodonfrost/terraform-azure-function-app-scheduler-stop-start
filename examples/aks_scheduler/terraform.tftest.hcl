run "create_test_infrastructure" {
  ...
  assert {
    condition     = module.stop_aks_cluster.service_plan_name == "spn-to-stop-${random_pet.suffix.id}"
  ...
  assert {
    condition     = module.start_aks_cluster.service_plan_name == "spn-to-start-${random_pet.suffix.id}"
    error_message = "Invalid service plan name"
  }
